...
<body>
	...
	<div class="container">
		...
		<form action="processUpdateNote" method="POST">
			...
			<br> <input type="hidden" name="id" value="${noteObject.id}">
			...
			<div class="form-group">
				...
				<textarea required class="form-control" style="height: 300px;"
					placeholder="Enter content" name="description">${noteObject.description}</textarea>
			</div>
				...
				<c:when test="${noteObject.done}">
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="checkbox" name="done"
							...
							for="inlineCheckbox1">Unmark if not Done</label>
					</div>
				</c:when>
				<c:otherwise>
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="checkbox" name="done">
						<label class="form-check-label" for="inlineCheckbox1">Mark
							as done</label>
					</div>
				</c:otherwise>
			</c:choose>
